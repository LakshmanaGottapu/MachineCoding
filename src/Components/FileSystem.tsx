...
function FileSystem({data, level=1, setActiveItem}:{data:FileType, level?:number, setActiveItem:Dispatch<SetStateAction<FileType | undefined>>}) {
    ...
    return (
        <div style={{marginLeft: '1rem', width:`${(20-level)}vw`}} onClick={displayContent}>
            ...
            {data.isFolder && folderOpen && data.items!=undefined && data.items.map((item, index) => <FileSystem key={index} data={item} level={level+1} setActiveItem={setActiveItem}/>)}
        </div>
    )
}
